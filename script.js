// Product Data based on images in folder
const products = [
...
function updateAuthState() {
    if (currentUser) {
        ...
        if (welcomeMsg) welcomeMsg.innerText = `Hi, ${currentUser.username}!`;
    } else {
...
function renderHistory() {
    ...
    if (!currentUser) {
        historyList.innerHTML = '<p class="text-center">Please log in to see your order history.</p>';
        return;
    }

    if (orderHistory.length === 0) {
        historyList.innerHTML = '<p class="text-center">You haven\'t placed any orders yet.</p>';
        return;
    }
